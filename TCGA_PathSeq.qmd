    ...
    group_modify(~ {
      ...
      map_dfr(cluster_pairs, function(cluster_pair) {
        cluster_data <- sub_data |> 
          filter(!!sym(cluster_col) |>  cluster_pair)
...
compare_species <- function(df, colonne_name = "name", colonne_score = "score_normalized", colonne_cluster = "cluster") {
  ...
  for (esp in especes) {
    ...
    if (length(unique(df_esp[[colonne_cluster]])) < 2) {
      message(paste("Espèce", esp, ": impossible de réaliser le test de Kruskal-Wallis, car toutes les observations sont dans le même groupe."))
...
#aucune comparaison significative p.adj > 0.05
...
positive_samples_selected |>
  count(tax_id)
all<- all |>
  rename(cluster = V1)
all<- all |> 
left_join(positive_samples, by= c('tax_id', 'cluster'))

cluster1_sbs <- all |> 
  filter(cluster == '5mC deamin') |>
  filter(V2 == 'SBS16-like') |> 
  filter(percentage > 25)
```

```{r}

#same test but at the genus level 

wilcox_results_genus <- wilcox_test(data = data_genus, 
                                  name_col ='name', 
                                  cluster_col = 'cluster', 
                                  score_col ='score_normalized') |>
                                  filter(p_value <= 0.05)

genus <- wilcox_results_genus |>
...
genus<- genus |>
  rename(cluster = V1)
genus<- genus |> 
left_join(positive_samples_genus, by= c('tax_id', 'cluster'))

genus_id_filtered <- genus |> 
  filter(percentage >25) |>
  pull(tax_id) |>
  unique()
```
...
```{r}
wilcox_tongue_genus <- wilcox_test(data = tongue_genus, 
                              name_col = "name", 
                              cluster_col = "cluster", 
                              score_col = "score_normalized")
...
```{r}
wilcox_tongue_species <- wilcox_test(data = tongue_species, 
                              name_col = "name", 
                              cluster_col = "cluster", 
                              score_col = "score_normalized")
